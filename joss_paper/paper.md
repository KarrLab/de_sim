...
These models will likely represent each component and interaction using mechanistic approximations that are derived from first principles and calibrated with data. For example, models of biochemical networks may use rate laws based on a combination of the Michaelis-Menten approximation and empirically-observed maximum reaction velocities.
Furthermore, large models that represent multiple types of components and their interactions will require diverse approximations and large, heterogeneous datasets. New tools are needed to build and simulate such large, data-intensive models.

One of the most promising methods for building and simulating large, data-intensive models is discrete-event simulation (DES). DES represents the dynamics of a system as a sequence of instantaneous events [@fishman2013discrete]. Many fields use DES for a broad range of research, such as studying the dynamics of biochemical networks, characterizing the performance of computer networks, evaluating potential war strategies, and forecasting epidemics [@banks2005discrete].
Although multiple DES tools exist, it remains difficult to build and simulate large data-intensive models.  First, it is cumbersome to create complex models with the low-level languages supported by many of the existing tools. Second, most of the existing tools are siloed from the ecosystems of data science tools that are exploding around Python and R.
...
DE-Sim helps researchers model complex systems by enabling researchers to use Python's powerful OO features to manage multiple types of components and multiple types of interactions.
By building upon Python, DE-Sim also makes it easy for researchers to use Python's powerful data science tools, such as pandas [@mckinney2010data] and SciPy [@virtanen2020scipy], and large, heterogeneous datasets to build comprehensive and detailed models.
...
* **Results logging:** DE-Sim makes it easy to log the results of simulations by providing an easily-configurable module for specifying which state variables should be monitored and how frequently they should be recorded.
...
Although there are several other DES tools, we believe that DE-Sim uniquely facilitates data-intensive modeling through a novel combination of OO modeling and support for numerous high-level data science tools. \autoref{fig:comparison} illustrates the strengths and weaknesses of DE-Sim compared to some of the most popular DES tools.

![**Comparison of DE-Sim with some of the most popular existing DES tools.**
DE-Sim is the only open-source, OO DES tool based on Python.
This combination of features makes DE-Sim uniquely suited to the creation and simulation of complex data-intensive models. 
\label{fig:comparison}](comparison.pdf)

SimPy is an open-source functional DES tool. As another Python-based tool, SymPy also makes it easy for researchers to leverage the Python ecosystem to build models. However, we believe that DE-Sim makes it easier for researchers to build complex models by enabling researchers to implement models as collections of classes rather than collections of functions. In addition, we believe the DE-Sim is simpler to use because DE-Sim provides a uniform approach for scheduling self and non-self events, whereas SimPy provides two different mechanisms for scheduling self and non-self events.
...
Create a subclass of `de_sim.EventMessage` to represent steps. Use the instance attribute `step_value` to capture the magnitude and direction of a step. Declare that the `step_value` instance attribute represents the data carried by the message by setting the value of the class attribute `msg_field_names` to a list with the single element `step_value`. The `msg_field_names` attribute is similar to the `field_names` parameter used by Python’s `namedtuple` factory function. Arguments to an `EventMessage`’s constructor are assigned one-to-one to attributes with the names in `msg_field_names`. The simulation objects which execute steps will use this `step_value` attribute to change the position of the random walk.
...
(The invariant says *non-decreasing* time order, and not *increasing* time order, because events can occur simultaneously, as discussed DE-Sim’s Jupyter notebooks.)
...
\label{fig:random_walk_trajectory}](random_walk_plot.pdf)

# Performance of DE-Sim

\autoref{fig:performance} illustrates the performance of DE-Sim simulating cyclic messaging networks of a range of sizes. These simulations represent rings of nodes which, upon receiving a message from their rear neighbor, send the same message to their forward neighbor with a one-time unit delay. Each simulation is initialized by sending a message to each node at the first time unit. 
...
![**Performance of DE-Sim simulating cyclic messaging networks of a range of sizes.** 
We executed each simulation for 100-time units. Each statistic represents the average of three simulation runs in a Docker container on a 2.9 GHz Intel Core i5 processor. 
...
Due to the diverse timescales of the subsystems inside cells, one promising way to simulate whole-cell models is to co-simulate slow subsystems with fined-grained algorithms and fast subsystems with coarse-grained algorithms. For example, slow subsystems, such as transcription, could be simulated with the Stochastic Simulation Algorithm (SSA, @gillespie1977exact), while faster subsystems, such as signal transduction, could be simulated with ordinary differential equations (ODEs). Metabolism, another fast process, could be simulated with Flux-Balance Analysis (FBA, @orth2010flux). However, there are no tools for co-simulating these algorithms beyond ad hoc implementations for specific models.

To accelerate whole-cell modeling, we are using DE-Sim to create WC-Sim.
First, we used NumPy and ODES scikit [@malengier2018odes] to implement simulation object subclasses for dFBA, ODE, and SSA simulations. These classes use attributes to represent the population of each molecular species and event messages to schedule reactions that update these populations. Second, we implemented an additional simulation object subclass that represents the populations of the molecular species shared by two or more sub-simulations. Third, we used event messages to synchronize the populations of these shared species among instances of the global species class and the dFBA, ODE, and SSA classes.

DE-Sim's OO functionality made it easy to implement classes for dFBA, ODE, and SSA, and DE-Sim's event messages made it easy to co-simulate these algorithms.
In addition, the ability to use NumPy made it easy to generate the random numbers needed to implement SSA, and the ability to use ODES scikit made it easy to use CVODE to implement the ODE simulation object class.
We anticipate that WC-Sim will help researchers conduct unprecedented simulations of cells.

# Conclusion

In summary, DE-Sim is an open-source, OO DES tool implemented in Python that makes it easier for modelers to create and simulate complex data-intensive models. First, DE-Sim makes it easy for researchers to use Python's OO features to manage multiple types of model components and their interactions. Second, DE-Sim makes it easy for researchers to use Python data science tools, such as pandas and SciPy, and large, heterogeneous datasets to calibrate complex models. Together, we anticipate that DE-Sim will accelerate the construction and simulation of unprecedented models of complex systems, leading to new scientific discoveries and engineering innovations.

To further advance the simulation of complex models, we aim to improve the simulation performance of DE-Sim. One potential direction is to use DE-Sim as a specification language for a parallel DES system such as ROSS [@carothers2000ross]. This combination of DE-Sim and ROSS would enable modelers to both create models with DE-Sim's high-level model specification semantics and quickly simulate large models with ROSS.
