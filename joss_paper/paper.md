...
This figure illustrates a space-time visualization of all of the events and messages in a simulation of the parallel hold (PHOLD) parallel DES benchmark model [@fujimoto1990performance] with three simulation objects. The timeline (grey line) for each object shows its events (grey dots). Each event in PHOLD schedules another event. The creation of each event is illustrated by an event message (arrow) sent from the earlier event to the event being scheduled. The curved blue arrows indicate events scheduled by a simulation object for itself, while the straight purple arrows indicate event messages sent to other simulation objects. The programs for the PHOLD model and for visualizing the trajectory of any simulation are available in the DE-Sim Git repository. 
...
Each tool provides a programming environment for developing DES models, a simulator that simulates models, and methods for recoding simulation predictions.
...
This combination of features makes it uniquely suitable for creating discrete-event models for studying complex systems. 
...
Its graphical interface enables easy description of queueing models of networks and processes.
...
SystemC [@mueller2001simulation] is an OO DES tool based on C++ commonly used to model the design and performance of digital systems.
...
An important benefit of OO DES tools like DE-Sim and other OO tools in \autoref{fig:comparison} is that individual simulation runs may be sped up by parallel execution on multiple cores.
More precisely, the simulation of an OO DES model composed of objects that only interact with each other via event messages and do not access shared memory might be sped up by distributing its objects across multiple cores and executing them in parallel [@schumacher2010parsc].
...
In summary, the primary advantage of DE-Sim is that It combines the power and convenience of OO modeling with the ability to leverage Python's library of data science tools to manage and analyze the large datasets needed by models of complex systems.
...
We illustrate this process with a model of a random walk on the integer number line (see an executable copy of this example in a [Jupyter notebook](https://sandbox.karrlab.org/notebooks/de_sim/1.%20DE-Sim%20tutorial.ipynb)). 
...
An event message class must be documented by a docstring.
...
The Python classes that generate and handle simulation events are simulation object classes, which are encoded as subclasses of `SimulationObject`.
A custom class creation method for `SimulationObject` gives special meaning to certain methods and attributes.
...
Finally, `run` simulates a model. It takes the maximum time to which the simulation can execute. `run` also takes several optional configuration arguments, as described in the DE-Sim [API documentation](https://docs.karrlab.org/de_sim/master/source/de_sim.html#de_sim.simulator.Simulator.simulate).
